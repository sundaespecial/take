...
That writes every `mipmap-*`/`drawable-*` density into `android/`. Check `android/app/src/main/res/mipmap-xxxhdpi/ic_launcher.png` actually shows the amber REC mark before building a release — the placeholder is a blue "X" and is easy to miss until it's live on a phone. The separate PWA icons at the repo root (`icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`) are *not* generated by that command; they're committed directly and are what the installed web app uses. `assets/store/` holds the Play Console listing graphics (feature graphic, social card), which aren't part of any build.
